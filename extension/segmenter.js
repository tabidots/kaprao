...
export class ThaiPrefixScanner {
    ...
    getSegmentation(text) {  // Remove 'node' parameter
        let cached = this.segCache.get(text);
        if (cached) {
        ...
        this.segCache.set(text, { segments });
        return segments;
    }

    // Segment compound surface (inner segmentation)
    segmentSurface(surface) {
    ...
    getAt(text, offset) {
        ...
        if (typeof text !== 'string') {
            throw new Error(`getAt: text parameter is ${typeof text}, expected string`);
        }
        ...
        const segments = this.getSegmentation(text);
        ...
        if (bestMatch.entries.some(e => e.is_compound)) {
            console.log('🔍 Compound found:', {
                surface: bestMatch.surface,
                surfaceType: typeof bestMatch.surface,
                bestMatch
            });
            const seg = this.segmentSurface(bestMatch.surface);
            if (seg && seg.length > 1) {
                innerSegments = seg;
            }
        }
